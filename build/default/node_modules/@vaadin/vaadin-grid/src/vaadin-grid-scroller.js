  ...
  _increasePoolIfNeeded(count) {
    ...
    if (!this._initialPoolCreated) {
      ...
      this._debounceIncreasePool = Debouncer.debounce(this._debounceIncreasePool, animationFrame, () => {
        ...
        const viewportCovered = this._physicalCount * this._physicalAverage >= this._viewportHeight * 3; // Don't increase the pool if there's enough rows. This is to avoid an eternal loop.

        if (!viewportCovered) {
          this._accessIronListAPI(() => super._increasePoolIfNeeded(count));

          if (count) {
            const childNodes = Array.from(this.$.items.childNodes); // Ensure the rows are in order after increasing pool
